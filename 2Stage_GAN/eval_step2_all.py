...
# save fmeasure
save_fmeasure = {
    'step2_normal':{'11':[[] for i in range(4)], '13':[[] for i in range(4)], '14':[[] for i in range(4)], '18':[[] for i in range(4)]}
}
...
prev_dibco_year = "18"#test_images[0][0].split('/')[-1].split('.')[0][7:9]
total_time = time.time()
for test_image, test_mask in test_images:

    img_name = test_image.split('/')[-1].split('.')[0]
    image = cv2.imread(test_image)
    dibco_year = "18"#img_name.split('-')[0][7:]
