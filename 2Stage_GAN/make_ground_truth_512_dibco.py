...
def main(argv=None):
    ...
    image_dirs = ['DIBCO_Orig', 'image_test']
    mask_dirs = ['DIBCO_GT', 'mask_test']
    ...
    # 
    image_save_dirs = ['dibco_resize_6_4/train/image', 'dibco_resize_6_4/test/image']
    mask_save_dirs = ['dibco_resize_6_4/train/mask', 'dibco_resize_6_4/test/mask']
    ...
    for image_dir, mask_dir, image_save_dir, mask_save_dir in zip(image_dirs, mask_dirs, image_save_dirs, mask_save_dirs):
        ...
        for image in image_list:
            ...
            elif os.path.isfile(os.path.join(mask_dir, img_name + '.tiff')):
                mask = cv2.imread(os.path.join(mask_dir, img_name + '.tiff'), cv2.IMREAD_GRAYSCALE)
            elif os.path.isfile(os.path.join(mask_dir, img_name + '.tif')):
                mask = cv2.imread(os.path.join(mask_dir, img_name + '.tif'), cv2.IMREAD_GRAYSCALE)
            else:
                print('no mask')
                continue
            ...
            image = cv2.resize(image, dsize=reshape, interpolation=cv2.INTER_LINEAR)
            mask = cv2.resize(mask, dsize=reshape, interpolation=cv2.INTER_LINEAR)
