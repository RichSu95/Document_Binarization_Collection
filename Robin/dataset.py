...
def parse_args():
    ...
    parser.add_argument('-i', '--input', type=str, default=os.path.join('.', 'data'),
                        help=r'directory with input train and ground-truth images (default: "%(default)s")')
    parser.add_argument('-o', '--output', type=str, default=os.path.join('.', 'input'),
...
def main():
    ...
    for i, fname in enumerate(glob.iglob(os.path.join(args.input, '**', '*_parts', '*_in.*'), recursive=True)):
        copy2(os.path.join(fname), os.path.join(args.output, 'in', str(i+6171) + '_in.png'))
        copy2(os.path.join(fname.replace('_in', '_gt')), os.path.join(args.output, 'gt', str(i+6171) + '_gt.png'))
